...
- auto-detecting the initial language of the client based on the system's language settings (if provided; happens whenever no language setting is saved yet);
...
> [!NOTE]
> The hardcoded client strings can be overridden using the built-in language translation. The built-in language code of this client distribution is **`zh`** (see `ProgramConstants.HARDCODED_LOCALE_CODE`), so create a `Resources/Translations/zh/Translation.ini` file to override hardcoded strings. Because the built-in language strings are always available, that language is picked by default even when the client has no other translations.
...
MapEncoding=UTF-8  ; string, defines the name of the map encoding to be used to load the map files to the spawnmap.ini file. Defaults to UTF-8 (without BOM). The 'Auto' option makes the client try to guess the encoding. Please either omit this line or specify 'UTF-8'. Only specify 'Auto' or an encoding different from 'UTF-8' if you really know what you are doing.
    ...
    - `[parent control name]` - the name of the parent control of the control that the value is defined for. Specifying `Global` instead of the parent name allows to specify identical translated value for all instances of the control regardless of the parent (parent-specific definition overrides this still though)
      - `[control name]` - the name of the control that the value is defined for.
        - `[attribute name]` - the name of the attribute that is being translated. Currently supported:
          ...
          - `$TextAnchor`, `$AnchorPoint` for labels using dynamic (expression) properties;
          - `$LeftClickAction` for the dynamic left-click action.
...
The main `Translation.ini` can include other INI files through the `[INISystem]` section, e.g. to reuse a shared `ClientTranslation.ini`:

```ini
[INISystem]
BasedOn=ClientTranslation.ini   ; comma-separated list of files, resolved relative to Translation.ini;
                                ; supports the $THEME_DIR$ variable. Values in Translation.ini override the included files.
```

## Ingame translation setup

The translation system's ingame translation support requires the mod/game author(s) to specify the files which translators can provide in order to translate the game. The files are specified in the `GameFileX=path/to/source.file,path/to/destination.file[,checked]` INI key syntax in the `[Translations]` section of `ClientDefinitions.ini` (X is any text you want to add to the key to help sort files), with comma-separated parts of the value meaning the following:
...
Each key must start with `GameFile` and contain 2 to 3 comma-separated parts (the third one, when present, must literally be `CHECKED`, case-insensitive). Invalid syntax throws an `IniParseException` at client startup.
...
1. Create a folder in `Resources/Translations` that uses the desired language code as name (see above) and place `Translation.ini` from `Client` folder there, and start translating the strings and uncommenting the translated ones.
   - Hardcoded strings are shared between same client binaries and are independent of mods, so you could reuse all the strings with `Client` prefix that you or someone else made for the language you're translating the client to. Or use `[INISystem]->BasedOn=` in the main `Translation.ini` to include a separate file (for instance, `ClientTranslation.ini`) with all the `Client`-prefixed strings placed in the same section.
