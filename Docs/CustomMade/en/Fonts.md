...
The client searches for `Fonts.ini` through its asset search paths, loading the first one found (in order):

1. Translation + theme folder (e.g. `Translations/ko-KR/Allied/Fonts.ini`)
2. Theme folder (e.g. `Resources/Allied/Fonts.ini`)
3. Shared theme folder (`Resources/Themes`)
4. Translation folder (e.g. `Translations/ko-KR/Fonts.ini`)
5. Base resources folder (`Resources/Fonts.ini`)
6. Game root directory

This lets translations supply their own fonts without touching the base configuration, and lets shared themes in `Resources/Themes` provide fonts without being overridden by a translation.
...
; When the ENTIRE section is absent, the defaults shown below are used.
; Note: if you write the section but omit a key, that key falls back to the
; per-key INI defaults (0, 0, 1) — see the reference table below.
KernelWidth=4
KernelHeight=4
...
|----------|-----------|-------------|
| `Type` | Both | `TrueType` or `SpriteFont` (default `SpriteFont`). |
...
| `Size` | TrueType | Font height in pixels (default `16`). This is the em-square height. The actual rendered height of characters may be slightly smaller depending on the font's metrics. Ignored for fallback fonts. |
...
|----------|---------|-------------|
| `Enabled` | `false` | Enable HarfBuzz text shaping. Required for complex scripts (Arabic, Hebrew, Hindi) and ZWJ emoji sequences. Disable for Latin-only or CJK-only text for better performance. If enabled but the native HarfBuzz library cannot be loaded, it is forced back to `false`. |
| `EnableBiDi` | `true` | Enable bidirectional text support for mixed LTR/RTL text. Only applies when `Enabled=true`. |
| `CacheSize` | `100` | Number of shaped-text cache entries. Use 1000 or more for CJK or other large-script languages. Values below 1 are reset to 100. |

#### `[FontRendering]`

The defaults in this table are used when the **entire** `[FontRendering]` section is absent (class defaults). If the section exists but an individual key is missing, that key uses the per-key INI fallback shown in parentheses.

| Property | Default (section absent) | Description |
|----------|--------------------------|-------------|
| `KernelWidth` | `4` (INI: `0`) | Horizontal blur kernel size applied by FontStashSharp when rasterizing glyphs. Non-negative. |
| `KernelHeight` | `4` (INI: `0`) | Vertical blur kernel size. Non-negative. |
| `FontResolutionFactor` | `5` (INI: `1`) | Multiplier for the internal glyph rasterization size. Values above `1` produce sharper output at the cost of a larger atlas. Non-negative. |
| `TextureWidth` | `1024` | Width of each FontStashSharp atlas page in pixels (minimum 1). |
| `TextureHeight` | `1024` | Height of each FontStashSharp atlas page in pixels (minimum 1). |
| `GlyphRenderResult` | `Premultiplied` | How glyph alpha is encoded: `Premultiplied` (matches a premultiplied-alpha `SpriteBatch`), `NonPremultiplied` (matches `AlphaBlend`), or `NoAntialiasing` (hard 1-bit edges for pixel-art fonts). Matching is case-insensitive; invalid values fall back to `Premultiplied`. |
